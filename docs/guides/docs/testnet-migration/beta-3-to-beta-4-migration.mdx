...
<<<<<<< HEAD
Fuel Wallet version `0.11.0` from the Chrome Web Store is not compatible with beta 4. For compatibility with beta-4, please download the non-backwards compatible Fuel Wallet version `0.12.2`.
=======
Fuel wallet version `0.11.0` is not compatible with beta 4. Please download the non backwards compatible Fuel wallet version `0.12.2` for beta-4 from the Chrome store.

```sh
npm install @fuel-wallet@0.12.2
>>>>>>> 00979533a2e0e0359137fe6e7cda64c730f1a5f1
