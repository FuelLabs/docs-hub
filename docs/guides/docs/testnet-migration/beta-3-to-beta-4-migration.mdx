...
In the Rust SDK, the `.get()` function has been superseded by either `.simulate()` or `.call()`. The `.simulate()` method is tailored for read-only calls and testing potential blockchain changes without committing them, while `.call()` should be used for actions that modify the blockchain's state, keeping in mind that state-altering methods using .call() will consume resources.
...
let { value } = await contract.functions.add_count().call();
```
