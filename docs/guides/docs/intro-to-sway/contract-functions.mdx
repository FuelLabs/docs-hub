...
Lastly, we can process the payment to the seller. It's recommended to transfer assets only after all storage modifications are completed to prevent [re-entrancy attacks](/docs/sway/blockchain-development/calling_contracts/#handling-re-entrancy).
