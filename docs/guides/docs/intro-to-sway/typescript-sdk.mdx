...
Wrap your components with `FuelProvider` from `@fuel-wallet/react` to enable Fuel's custom React hooks for wallet functionalities.
...
The state variable called `wallets` with the `useState` hook will have the type `WalletLocked`.
