...
Sometimes you might end up using multiples toolchain at once, don't worry if you get confused about which toolchain your are using since you can check your current toolchain anytime.

👉 Run the `fuelup show` command to see toolchain that you are using and the version of the binaries of that toolchain.
...
It's always possible to run a development environment on the browser. 

> 👉 Please visit our guide on [Github Codespaces](guides/installation/codespace) to use the Fuel toolchain on the browser.