...
Looking at the predicate, you can immediately notice several differences. There is no ABI or implementation, but simply a main function that returns true or false. And, of course, the predicate keyword is used to identify that the program is a predicate.
...
Now that we have a basic understanding of what a predicate is, we can start writing our predicate.