...
To modify the intial state of the chain, you must configure a `chainConfig.json` file. 
...
You can edit the `coins` array inside the `inital_state` object to modify the intial assets owned by a given address.
...
To start the node with a custom configration, you can use the command below:
...
To run a local node with persistence a `chainConfig.json` file is required. 
You can find an example above under "Funding a wallet locally".
