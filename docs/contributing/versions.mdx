...
- The default set targets the named `mainnet` Fuelup channel. Fuelup's undated `latest` channel is a network alias — at the time of writing it resolves to `mainnet` (verify against the [Fuelup channel reference](https://install.fuel.network/master/concepts/channels.html)); it does not mean the newest upstream release.
