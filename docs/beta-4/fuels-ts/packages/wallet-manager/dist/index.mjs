...
var MnemonicVault = class {
  constructor(options) {
    ...
    __privateSet(this, _secret, options.secret || Mnemonic.generate());
...
import { Wallet as Wallet2 } from "@fuel-ts/wallet";
var _privateKeys;
var PrivateKeyVault = class {
  ...
  getPublicAccount(privateKey) {
    const wallet = Wallet2.fromPrivateKey(privateKey, this.provider);
    return {
      address: wallet.address,
      publicKey: wallet.publicKey
    };
  ...
  exportAccount(address) {
    const privateKey = __privateGet(this, _privateKeys).find(
      (pk) => Wallet2.fromPrivateKey(pk, this.provider).address.equals(address)
    );
    if (!privateKey) {
      throw new FuelError2(
        ErrorCode2.WALLET_MANAGER_ERROR,
  ...
  getWallet(address) {
    const privateKey = this.exportAccount(address);
    return Wallet2.fromPrivateKey(privateKey, this.provider);
  }
};
...
function assert(condition, message) {
  if (!condition) {
    throw new FuelError3(ErrorCode3.WALLET_MANAGER_ERROR, message);
  }
}
...
var Vault = class {
  constructor(_options) {
    throw new FuelError4(ErrorCode4.NOT_IMPLEMENTED, "Not implemented.");
  }
  serialize() {
    throw new FuelError4(ErrorCode4.NOT_IMPLEMENTED, "Not implemented.");
  }
