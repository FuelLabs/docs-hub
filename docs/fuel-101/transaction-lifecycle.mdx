...
The transaction is run through an initial set of [validation checks](/docs/specs/protocol/tx-validity/#vm-precondition-validity-rules), such as [predicate verification](/docs/specs/fuel-vm/#predicate-verification), [signature validation](/docs/specs/protocol/tx-validity/#valid-signatures), and making sure the transaction doesn't "double spend" an input that is already in use from another transaction in the pool.
...
The block producer creates a new block through a [`Mint`](/docs/specs/tx-format/transaction/#transactionmint) type transaction that includes the transaction we are following here. Each block contains one `Mint` type transaction at the end of the block.
